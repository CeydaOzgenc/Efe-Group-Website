...
<head>
  <meta charset="UTF-8">
  <title>Efe Group | Referanslar</title>
...
<div class="container main">
  <div class="row">
    <?php $say=0; $rows = mysqli_fetch_all($sql, MYSQLI_ASSOC);
     foreach ($rows as $referans) {
      if($say==0){$idNext=$say+1; $idBack=count($rows)-1;}
      elseif ($say==count($rows)-1) {$idNext=0; $idBack=$say-1;}
      else{$idNext=$say+1; $idBack=$say-1;}?>
      <div class="git" id="<?php echo $referans['Reference_id'];?>">
        <div class="resim">
          <a onClick="kapat(<?php echo $referans['Reference_id'];?>)" class="close">X</a>
          <a class="icon n1" onClick="gallerygo(<?php echo $rows[$idBack]['Reference_id']?>,<?php echo $referans['Reference_id'];?>)"><</a>
          <img class="gimg" src="<?php echo "img/referans/".$referans['Reference_img'];?>">
          <a class="icon" onClick="gallerygo(<?php echo $rows[$idNext]['Reference_id']?>,<?php echo $referans['Reference_id'];?>)">></a>
        </div>
      </div>
      <div class="col-sm-6 col-lg-4">
        <div class="zoombox">
          <img src="<?php echo "img/referans/".$referans['Reference_img'];?>" class="zoomboximg">
          <div class="zoomboxDiv">
             <div class="center_ozel">
                <span>
                    <i class='fas fa-expand-alt' onclick=" <?php echo 'gallery('.$referans['Reference_id'].')'?> "></i>
                </span>
             </div>
          </div>
        </div>
