...
<head>
  <meta charset="UTF-8">
  <title>Efe Group | Sertifikalar</title>
...
<?php include "admin/database.php";
  $sql=mysqli_query($db,"select * from seltificas");
?>
</head>
...
<div class="container main">
  <div class="row">
    <?php $say=0; $rows = mysqli_fetch_all($sql, MYSQLI_ASSOC);
     foreach ($rows as $seltifica) {
      if($say==0){$idNext=$say+1; $idBack=count($rows)-1;}
      elseif ($say==count($rows)-1) {$idNext=0; $idBack=$say-1;}
      else{$idNext=$say+1; $idBack=$say-1;}?>
      <div class="git" id="<?php echo $seltifica['Seltificas_id'];?>">
        <div class="resim">
          <a onClick="kapat(<?php echo $seltifica['Seltificas_id'];?>)" class="close">X</a>
          <a class="icon n1" onClick="gallerygo(<?php echo $rows[$idBack]['Seltificas_id']?>,<?php echo $seltifica['Seltificas_id'];?>)"><</a>
          <img class="gimg" src="<?php echo "img/seltifica/".$seltifica['Seltificas_img'];?>">
          <a class="icon" onClick="gallerygo(<?php echo $rows[$idNext]['Seltificas_id']?>,<?php echo $seltifica['Seltificas_id'];?>)">></a>
        </div>
      </div>
      <div class="col-sm-6 col-lg-4">
        <div class="zoombox">
          <img src="<?php echo "img/seltifica/".$seltifica['Seltificas_img'];?>" class="zoomboximg">
          <div class="zoomboxDiv">
             <div class="center_ozel">
                <span>
                    <i class='fas fa-expand-alt' onclick=" <?php echo 'gallery('.$seltifica['Seltificas_id'].')'?> "></i>
                </span>
             </div>
          </div>
        </div>
      ...
      <div class="row">
        ...
        <div class="col-sm-9">
          <ul class="footer_nav">
            ...
            <li><a href="seltifikalar.php">Sertifikalar</a></li>
