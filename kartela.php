...
<?php include "admin/database.php";
  $sql=mysqli_query($db,"select * from kartela");
?>
</head>
...
<div class="container main">
  <div class="row">
    <?php $say=0; $rows = mysqli_fetch_all($sql, MYSQLI_ASSOC);
     foreach ($rows as $kartela) {
      if($say==0){$idNext=$say+1; $idBack=count($rows)-1;}
      elseif ($say==count($rows)-1) {$idNext=0; $idBack=$say-1;}
      else{$idNext=$say+1; $idBack=$say-1;}?>
      <div class="git" id="<?php echo $kartela['Kartela_id'];?>">
        <div class="resim">
          <a onClick="kapat(<?php echo $kartela['Kartela_id'];?>)" class="close">X</a>
          <a class="icon n1" onClick="gallerygo(<?php echo $rows[$idBack]['Kartela_id']?>,<?php echo $kartela['Kartela_id'];?>)"><</a>
          <img class="gimg" src="<?php echo "img/kartela/".$kartela['Kartela_img'];?>">
          <a class="icon" onClick="gallerygo(<?php echo $rows[$idNext]['Kartela_id']?>,<?php echo $kartela['Kartela_id'];?>)">></a>
        </div>
      </div>
      <div class="col-sm-6 col-lg-4">
        <div class="zoombox">
          <img src="<?php echo "img/kartela/".$kartela['Kartela_img'];?>" class="zoomboximg">
          <div class="zoomboxDiv">
             <div class="center_ozel">
                <span>
                    <i class='fas fa-expand-alt' onclick=" <?php echo 'gallery('.$kartela['Kartela_id'].')'?> "></i>
                </span>
             </div>
          </div>
        </div>
