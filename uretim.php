...
<?php include "admin/database.php";
  $sql=mysqli_query($db,"select * from production");
?>
</head>
...
<div class="container main">
  <div class="row">
    <?php $say=0; $rows = mysqli_fetch_all($sql, MYSQLI_ASSOC);
     foreach ($rows as $uretim) {
      if($say==0){$idNext=$say+1; $idBack=count($rows)-1;}
      elseif ($say==count($rows)-1) {$idNext=0; $idBack=$say-1;}
      else{$idNext=$say+1; $idBack=$say-1;}?>
      <div class="git" id="<?php echo $uretim['Production_id'];?>">
        <div class="resim">
          <a onClick="kapat(<?php echo $uretim['Production_id'];?>)" class="close">X</a>
          <a class="icon n1" onClick="gallerygo(<?php echo $rows[$idBack]['Production_id']?>,<?php echo $uretim['Production_id'];?>)"><</a>
          <img class="gimg" src="<?php echo "img/uretim/".$uretim['Production_img'];?>">
          <a class="icon" onClick="gallerygo(<?php echo $rows[$idNext]['Production_id']?>,<?php echo $uretim['Production_id'];?>)">></a>
        </div>
      </div>
      <div class="col-sm-6 col-lg-4">
        <div class="zoombox">
          <img src="<?php echo "img/uretim/".$uretim['Production_img'];?>" class="zoomboximg">
          <div class="zoomboxDiv">
             <div class="center_ozel">
                <span>
                    <h4><?php echo $uretim['Production_text']?></h4>
                    <i class='fas fa-expand-alt' onclick=" <?php echo 'gallery('.$uretim['Production_id'].')'?> "></i>
                </span>
             </div>
          </div>
        </div>
