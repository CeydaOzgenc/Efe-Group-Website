...
<?php
    ...
    if(!$_SESSION)
    {
        ...
        $urunid=$_GET['urun'];
        $urunleryaz=mysqli_query($db,"select * from product where ProductMain_id=".$urunid);
        $urunler=mysqli_query($db,"select * from product_main");
        $anaurun=mysqli_fetch_array(mysqli_query($db,"select * from product_main where ProductMain_id=".$urunid));
?>
<html>
    <head>
        <meta charset="utf-8" />
                                ...
                                <div class="card text-white mb-4">
                                    ...
                                    <div class="card-body">
                                        <table id="datatablesSimple">
                                            ...
                                            <tbody>
                                            <?php while ($urunyaz=mysqli_fetch_array($urunleryaz)){ ?>
                                                <tr>
                                                    <td><img class="blog_img" src="<?php echo "../img/urunler/".$anaurun['ProductMain_name']."/".$urunyaz['Product_img']; ?>" /></td>
                                                    <td><?php echo $urunyaz['Product_text']; ?></td>
                                                    <td>
                                                        <a href="<?php echo "duzen.php?yer=ud&duzen=".$urunyaz["Product_Id"]."&id=".$urunyaz["ProductMain_id"];?>">
                                                            <input class="btn btn-primary" type="submit" value="Düzenle">
                                                        </a>
                                                        <a href="<?php echo 'sil.php?yer=ud&sil='.$urunyaz["Product_Id"] ?>">
                                    ...
                                    <div class="card-footer bg-primary d-flex align-items-center justify-content-between">
                                        <a class="small text-white" href="<?php echo 'ekle.php?yer=ud&id='.$urunid; ?>" >Yeni Ekle</a>
